...
tree-edit distance & GumTree~\cite{falleri2014gumtree} & medium--high & medium--high & future \\
graph kernels & Weisfeiler-Lehman~\cite{shervashidze2011weisfeiler}
  & medium & high & future \\
symbolic execution & KLEE~\cite{cadar2008klee}, angr~\cite{shoshitaishvili2016sok}
  & very high & high & future \\
data-flow analysis & Soufflé~\cite{jordan2016souffle}
  & high & high & future \\
code embeddings & CodeBERT~\cite{feng2020codebert} & medium & statistical only
  & not used \\
\bottomrule
\end{tabular}
\end{table}
...
\texttt{chardet-relicense/manuscript/figures/scripts/validation\_report.json}.
...
\texttt{verivus-oss/agent-assurance} under
\texttt{chardet-relicense/proof-bundle/}~\cite{verivus2026dagtoml}.
Reproduction:
...
bash agent-assurance/chardet-relicense/proof-bundle/detect.sh
\end{lstlisting}
...
The manuscript was drafted by Claude Opus 4.7 (Anthropic), via the
Claude Code agent harness, working from the bundle commit at HEAD
\texttt{220cff4} in the \texttt{verivus-oss/agent-assurance}
repository. The proof bundle itself was authored over several
...
\texttt{chardet-relicense/manuscript/USER\_PROMPTS.md} in the same repository. The file is
