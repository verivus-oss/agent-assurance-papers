...
$43.5\%$); the v6/v7 match \emph{rate} is the lowest of the three,
...
\textbf{Per-function shape similarity ranks the three pairs and
is the cleanest single-signal calibration result.} C06f gives
...
$0.796\,[0.75,\,0.84]$ over $77/177=43.5\%$. The three CIs are
...
than v5/v6 do, and v6 and charset\_normalizer share none. C06b
...
($43.5\%$) with the lowest within-match similarity
...
bounds, and upper bounds. The full reproduction artefact is in
...
corrective-program spec (\cref{sec:llm-review}). Codex (OpenAI
Codex v0.132.0, GPT-5-class, \texttt{workspace-write} sandbox,
\texttt{on-request} approvals) iterated three rounds on the
bundle and two rounds on the paper, reaching UNCONDITIONAL
APPROVAL on all 15 bundle contracts and on the manuscript after
all blockers were addressed. Grok (Grok Build, model
\texttt{grok-build}, \texttt{bypassPermissions},
\texttt{alwaysApprove}) reached UNCONDITIONAL APPROVAL on all 15
bundle contracts in one round and on all 20 paper contracts after
a second round. Gemini (Gemini 3.1 Pro Preview, \texttt{yolo})
reached UNCONDITIONAL APPROVAL on the first-round bundle review;
its second-round bundle review and both paper-review rounds
failed before completion with an upstream \texttt{TerminalQuotaError},
so Gemini's verdict on the final paper version is not part of the
record at submission time. We note the gap explicitly rather than
silently omitting it. All three reviewers shared the same MCP set
